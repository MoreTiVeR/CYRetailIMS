...
//datatable = $("#tbItemTransferShortageReport").DataTable({
...
//        $('.dataTables_filter').appendTo("#tbItemTransferShortageReport");
...
datatable = $("#tbItemTransferShortageReport").DataTable({
    ...
    "ordering": true,
    "ajax": {
        "url": "/Report/GetItemTransferShortageReport",
        "type": "GET",
        "datatype": "json"
    },
    "columns": [
        ...
        { "data": "subitemtypename" },
        { "data": "brandname" },
        { "data": "suggestrefillqtybysystem" },
        { "data": "refillqty" },
        {
            "data": { shortageqty: "shortageqty" },
            "render": function (data) {
                var _shortageqty = parseInt(data.shortageqty);
                if (_shortageqty > 0) {
                    return "<span class='badges bg-lightyellow'>" + data.shortageqty + "</span>";
                }
                else {
                    return data.shortageqty;
                }
            }
        }
    ],
...
$("#btnSearch").on('click', function (event) {
    ...
    var valSubItemType = $("#ddlSubItemType").val();
    var subitemtypeid = parseInt(valSubItemType);

    var reqdata = { "startdate": startdate, "enddate": enddate, "branchid": branchid, "subitemtypeid": subitemtypeid };
    var jsonreqdata = JSON.stringify(reqdata);
    console.log(jsonreqdata);
    var request = $.ajax({
        type: 'POST',
        url: '/Report/SearchItemTransferShortageReport',
        data: jsonreqdata,
        ...
        success: function (response) {
            ...
            $("#tbItemTransferShortageReport").DataTable().clear().rows.add(response.data).draw();
            HideLoading();
        },
