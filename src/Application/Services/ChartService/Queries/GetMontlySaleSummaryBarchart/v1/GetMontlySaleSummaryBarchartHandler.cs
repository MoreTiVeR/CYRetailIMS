...
public class GetMontlySaleSummaryBarchartHandler : BaseService, IRequestHandler<GetMontlySaleSummaryBarchartQuery, BaseResponse<List<GetMontlySaleSummaryBarchartResponseDTO>>>
{
    public GetMontlySaleSummaryBarchartHandler(IMapper mapper, IUnitOfWork unitOfWork) : base(mapper, unitOfWork)
    {
    }

    public async Task<BaseResponse<List<GetMontlySaleSummaryBarchartResponseDTO>>> Handle(GetMontlySaleSummaryBarchartQuery request, CancellationToken cancellationToken)
    {
