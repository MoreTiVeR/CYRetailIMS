...
public class CreateItemInBranchHandlerV2 : BaseService, IRequestHandler<CreateItemInBranchListCommandV2, BaseResponse<CommandResponse>>
{
    public CreateItemInBranchHandlerV2(IMapper mapper, IUnitOfWork unitOfWork) : base(mapper, unitOfWork)
    {
    }

    public async Task<BaseResponse<CommandResponse>> Handle(CreateItemInBranchListCommandV2 request, CancellationToken cancellationToken)
    {
        ...
        if (!isNewImport)
        {
            List<CreateItemInBranchDetailCommandV2> updateItemEntity = request.items.Where(w => w.isupdate).ToList();
        ...
        List<CreateItemInBranchDetailCommandV2> newItemEntity = request.items.Where(w => !w.isupdate).ToList();
