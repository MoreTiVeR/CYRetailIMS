...
public class TTReceiptCreateEventHandler : INotificationHandler<TTReceiptCreateEvent>
{
    private readonly ILog4NetLogger _logger;
    public TTReceiptCreateEventHandler(ILog4NetLogger log4NetLogger)
    {
        _logger = log4NetLogger;
    }
    public Task Handle(TTReceiptCreateEvent notification, CancellationToken cancellationToken)
    {
        _logger.Info($"TTReceiptCreateEventHandler: {notification.GetType().Name}");
